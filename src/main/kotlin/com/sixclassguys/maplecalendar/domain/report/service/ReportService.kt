...
import com.sixclassguys.maplecalendar.global.exception.AlreadyReportBossPartyChatMessageException
import com.sixclassguys.maplecalendar.global.exception.BossPartyChatMessageNotFoundException
import com.sixclassguys.maplecalendar.global.exception.InvalidBossPartyChatReportException
...
class ReportService(
    ...
    fun reportChat(userEmail: String, chatId: Long, reason: ReportReason, detail: String?) {
        ...
        if (reportedMemberId == reporter.id) {
            throw InvalidBossPartyChatReportException("자신의 메시지는 신고할 수 없어요.")
        }

        // 3. 중복 신고 방지
        if (reportRepository.existsByReporterMemberIdAndTargetChatId(reporter.id, chatId)) {
            throw AlreadyReportBossPartyChatMessageException()
        }
